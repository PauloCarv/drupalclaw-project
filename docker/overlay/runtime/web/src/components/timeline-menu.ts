...
export function TimelineMenu({
        ...
        const update = () => {
            if (workspaceOpen) {
                ...
                if (sidebar) {
                    ...
                    // Offset right so we don't cover the sidebar's own
                    // ≡ menu button (workspace-menu-button), which lives
                    // at the left edge of the sidebar header.
                    setPos({ top: r.top + 8, left: r.left + 44 });
                }
            } else {
                setPos({ top: 8, left: 8 });
            }
    ...
    useEffect(() => {
        ...
        // Hide completely when the Workspace sidebar is open — the sidebar
        // already has its own ≡ menu and the two buttons collide visually.
        // The sidebar menu gains an "Open Dev Panel" item via the overlay
        // on workspace-explorer.ts, so no entry is lost.
        portalRef.current.style.display = workspaceOpen ? 'none' : '';
    ...
    const content = html`
        ...
        ${open && html`
            <div class="workspace-menu-dropdown timeline-menu-dropdown" ref=${menuRef} role="menu">
                ...
                <button class="workspace-menu-item" role="menuitem" onClick=${() => run(() => window.dispatchEvent(new CustomEvent('piclaw:open-dev-panel')))}>Open Dev Panel</button>
