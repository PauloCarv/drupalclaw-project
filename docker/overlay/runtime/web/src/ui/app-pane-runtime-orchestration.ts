...
export function usePaneRuntimeOrchestration(options: UsePaneRuntimeOrchestrationOptions) {
  ...
  const openDevPanelTab = useCallback(() => {
    openEditor('piclaw://dev-panel', { label: 'Dev Panel' });
  }, [openEditor]);

  useEffect(() => {
    const handler = () => { openDevPanelTab(); };
    window.addEventListener('piclaw:open-dev-panel', handler as EventListener);
    return () => window.removeEventListener('piclaw:open-dev-panel', handler as EventListener);
  }, [openDevPanelTab]);
